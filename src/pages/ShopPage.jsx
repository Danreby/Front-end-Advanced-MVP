...
export default function ShopPage({language}){
  ...
  return (
    <section id="shop" className="min-h-[60vh] p-6 max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-green-100">{t.title}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {sampleProducts.map(p => (
          <article key={p.id} className="bg-white/5 p-4 rounded-lg shadow-md border border-white/5">
            ...
            <h3 className="font-semibold mb-2 text-gray-100">{p.name}</h3>
            <div className="text-amber-300 font-bold mb-4">{p.price}</div>
